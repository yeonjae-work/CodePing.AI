...
def demo_commit_retrieval(manager):
    ...
    if commit_with_diffs:
        ...
        for i, diff in enumerate(commit_with_diffs.diffs, 1):
            ...
            print(f"      +{diff.additions}/-{diff.deletions} (내용 있음: {diff.has_content})")
    else:
...
def demo_recent_commits(manager):
    ...
    if recent_commits:
        ...
        for i, commit in enumerate(recent_commits, 1):
            ...
            print(f"      📊 {commit.diff_count}개 파일, +{commit.total_additions}/-{commit.total_deletions}")
            print()
    else:
        print("📭 저장된 커밋이 없습니다.")
...
def demo_multiple_commits(manager):
    ...
    for i, commit_info in enumerate(commits_to_store, 1):
        ...
        diff_data = []
        for j, (file_path, (adds, dels)) in enumerate(zip(commit_info["files"], commit_info["stats"])):
            diff_data.append(DiffData(
                file_path=file_path,
                additions=adds,
                deletions=dels,
...
def demo_compression():
    ...
    print(f"💾 저장 위치: {'DB' if len(compressed) <= 256*1024 else 'S3 (if configured)'}")
    print()
