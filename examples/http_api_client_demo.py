...
def performance_demo():
    ...
    client_no_cache = HTTPAPIClient(
        platform=Platform.GITHUB, auth_token=github_token, enable_cache=False
    )
    ...
    client_with_cache = HTTPAPIClient(
        platform=Platform.GITHUB, auth_token=github_token, enable_cache=True
    )
...
def rate_limiting_demo():
    ...
    client = HTTPAPIClient(
        platform=Platform.GITHUB, auth_token=github_token, enable_rate_limiting=True
    )
    ...
    try:
        ...
        for i in range(5):
            ...
            if response.success:
                ...
                if rate_limit:
                    print(
                        f"요청 {i+1}: 남은 요청 수 {rate_limit.remaining}/{rate_limit.limit}"
                    )
                else:
                    print(f"요청 {i+1}: 성공")
