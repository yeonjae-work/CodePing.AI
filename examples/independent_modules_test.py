...
def create_sample_webhook_payload():
    ...
    return {
        ...
        "repository": {
            ...
            "owner": {"name": "test-user", "login": "test-user"},
        },
...
def test_independent_modules():
    ...
    try:
        ...
        logger.info(
            f"   ✅ Lines: +{parsed_data.diff_stats.total_additions}/-{parsed_data.diff_stats.total_deletions}"
        )
        ...
        logger.info(
            f"   ✅ 분석 시간: {analysis_result.analysis_duration_seconds:.3f}초"
        )
        ...
        logger.info(
            f"   - Code Changes: +{analysis_result.total_additions}/-{analysis_result.total_deletions}"
        )
        ...
        logger.info(
            f"   - Analysis Duration: {analysis_result.analysis_duration_seconds:.3f}s"
        )
