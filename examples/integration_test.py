...
def test_gitdataparser_integration():
    ...
    try:
        ...
        for file_change in parsed_data.file_changes:
            print(
                f"     - {file_change.filename} ({file_change.file_type}): +{file_change.additions} -{file_change.deletions}"
            )
    ...
    try:
        ...
        commit_metadata = CommitMetadata(
            ...
            author_email=(
                first_commit.author.email if first_commit else "unknown@example.com"
            ),
        ...
        for analyzed_file in analysis_result.analyzed_files:
            ...
            print(
                f"       복잡도 변화: {analyzed_file.complexity_analysis.metrics.complexity_delta:+.2f}"
            )
            if analyzed_file.structural_analysis.changes.functions_added:
                print(
                    f"       추가된 함수: {analyzed_file.structural_analysis.changes.functions_added}"
                )
            if analyzed_file.structural_analysis.changes.classes_added:
                print(
                    f"       추가된 클래스: {analyzed_file.structural_analysis.changes.classes_added}"
                )
...
def test_language_detection_upgrade():
    ...
    for filename in test_files:
        ...
        print(
            f"{upgrade_indicator} {filename:15s} → {basic_type:10s} vs {detected_type:10s}"
        )
