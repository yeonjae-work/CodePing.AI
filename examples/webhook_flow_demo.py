...
def create_realistic_webhook_payload():
    ...
    return {
        ...
        "repository": {
            ...
            "owner": {"name": "testuser", "login": "testuser"},
            "default_branch": "main",
        },
...
async def simulate_webhook_flow():
    ...
    try:
        ...
        logger.warning(
            "⚠️ GitDataParser: API call failed (expected in demo): %s", str(e)[:100]
        )
        # 데모용 mock 데이터 생성
        ...
        parsed_data = ParsedWebhookData(
            ...
            diff_stats=DiffStats(
                total_additions=45, total_deletions=8, files_changed=5
            ),
            timestamp=datetime.now(),
        )
    ...
    try:
        ...
        logger.info(
            "   - Languages: %s", list(analysis_result.language_breakdown.keys())
        )
        logger.info(
            "   - Analysis duration: %.3fs", analysis_result.analysis_duration_seconds
        )

    except Exception as e:
        logger.warning(
            "⚠️ DiffAnalyzer: Analysis failed (expected with mock data): %s",
            str(e)[:100],
        )
        # 기본 분석 결과 시뮬레이션
    ...
    logger.info(
        "   - INSERT INTO analysis_results (commit_id, complexity_delta, languages)"
    )

    logger.info("☁️ S3 operations:")
    logger.info(
        "   - Upload diff content (compressed): 1.2KB → 0.3KB (75% compression)"
    )
