...
@click.option(
    "--format",
    type=click.Choice(["markdown", "json", "plain_text"]),
    default="markdown",
    help="출력 형식",
)
@click.option(
    "--strategy",
    type=click.Choice(["full_sync", "incremental"]),
    default="incremental",
    help="동기화 전략",
)
...
@click.option(
    "--filter-by",
    help='관계 필터링 (속성명:페이지ID 형식, 예: "프로젝트 마스터:21c18a4c52a1804ba78ddbcc2ba649d4")',
)
@click.pass_context
def add_database(
    ctx, database_id, name, output, format, strategy, transformer, filter_by
):
    """데이터베이스를 동기화 대상에 추가"""
    asyncio.run(
        _add_database(
...
async def _add_database(
    ...
    if filter_by:
        try:
            ...
            click.echo(
                f"🔍 관계 필터 설정: {property_name.strip()} → {page_id.strip()}"
            )
        except ValueError:
            click.echo(
                "❌ 잘못된 필터 형식입니다. '속성명:페이지ID' 형식으로 입력하세요."
            )
            return
...
async def _sync(config_file, target_filter):
    ...
    if target_filter:
        targets_to_sync = [t for t in config.targets if target_filter in [t.id, t.name]]
...
async def _quick_page(page_id, output_file, format):
    ...
    success = await quick_sync_page(token, page_id, output_file, ContentFormat(format))
