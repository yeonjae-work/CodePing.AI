...
def setup_detailed_logging():
    ...
    flow_formatter = logging.Formatter(
        "🔄 %(asctime)s - FLOW - %(message)s", datefmt="%H:%M:%S"
    )
...
class ModuleIOLogger:
    ...
    def __init__(self, module_name: str):
        ...
        self.logger = module_loggers.get(
            module_name, logging.getLogger(f"modules.{module_name.lower()}")
        )
    ...
    def log_input(self, operation: str, data: Any = None, metadata: Dict = None):
        ...
        # Flow 로거에도 기록
        log_module_io(
            self.module_name, f"{operation}_INPUT", input_data=data, metadata=metadata
        )
    ...
    def _summarize_data(self, data: Any, max_items: int = 5) -> str:
        ...
        try:
            if isinstance(data, dict):
                ...
                more = (
                    f" (+{len(data) - max_items} more)" if len(data) > max_items else ""
                )
                ...
                more = (
                    f" (+{len(data.__dict__) - max_items} more)"
                    if len(data.__dict__) > max_items
                    else ""
                )
        ...
        if isinstance(data, dict):
            ...
            try:
                if len(data) > 10:  # 리스트가 너무 길면 처음 10개만 표시
                    return [
                        simplify_data(item, max_depth, current_depth + 1)
                        for item in data[:10]
                    ] + [f"... (+{len(data) - 10} more items)"]
                return [
                    simplify_data(item, max_depth, current_depth + 1) for item in data
                ]
            except Exception:
            ...
            try:
                ...
                if isinstance(obj_dict, dict):
                    simplified_dict = simplify_data(
                        obj_dict, max_depth, current_depth + 1
                    )
                    if isinstance(simplified_dict, dict):
                        return {"__type": type(data).__name__, **simplified_dict}
                    else:
                        return {
                ...
                return {"__type": type(data).__name__, "__dict_access_error": str(e)}
...
def log_module_io(
    ...
    flow_logger.info(
        "MODULE_FLOW: %s",
        json.dumps(
            log_entry, ensure_ascii=False, indent=2, default=default_json_serializer
        ),
    )
...
def log_processing_chain_end(success: bool, metadata: Dict = None):
    ...
    log_module_io("PROCESSING_CHAIN", status, metadata=metadata or {})
